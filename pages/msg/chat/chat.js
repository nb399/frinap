...
Page({
  ...
  onLoad: function(options) {
    ...
    var userIdentity = options.identity;
    var userId = options.user_id;
    var userInfo = wx.getStorageSync('userinfo');
    var myAvatarUrl = userInfo.avatarUrl;
    var yourAvartarUrl = options.avatarUrl;
    var that = this
  ...
  _goToPerson() {
    ...
    var userIdentity = that.data.userIdentity;
    wx.navigateTo({
      url: '/pages/children_detail/children_detail?child_id=' + userId + '&identify=' + userIdentity + '&from=' + 'chat',
    })
  },
  bindMessage: function(e) {
    this.data.sendInfo = e.detail.value;
    this.setData({
  ...
  playRecord: function(e) {
    ...
    if (that.data.playing) {
      if (that.innerAudioContext.src == res.tempFilePath) {
        return 0;
      } else {
        wx.hideToast()
  ...
  sendMessage: function() {
    ...
    chatM.check(that.data.sendInfo).then(res=>{
      console.log(res)
      if (!res.status) {
        wx.showToast({
          title: res.message,
        ...
        if (msg) {
          ...
          that.data.chatMsg.push(msgData)
          if (!wx.getStorageSync(that.data.yourname + myName)) {
            var chatList = wx.getStorageSync('chatList') ? wx.getStorageSync('chatList') : [];
            chatList.push(msg.body.to);
  ...
  receiveMsg: function(msg, type) {
    ...
    if (msg.from == that.data.yourname || msg.to == that.data.yourname) {
      if (type == 'txt') {
        ...
        var options = {
          ...
          success: function(res) {
            ...
            var value = WebIM.parseEmoji(msg.url.replace(/\n/mg, ''))
            var msgData = {
              info: {
                from: msg.from,
                to: msg.to
      ...
      var time = WebIM.time()
      if (type == 'img' || type == 'location')
        var value = WebIM.parseEmoji((msg.url || msg.addr).replace(/\n/mg, ''))
      var msgData = {
        info: {
          from: msg.from,
          to: msg.to
